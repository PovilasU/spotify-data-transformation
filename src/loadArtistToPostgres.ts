...
const filePath = path.join(__dirname, "..", "data", "artists.csv");
...
async function loadCSVToPostgres() {
  try {
    ...
    const createTableQuery = `CREATE TABLE IF NOT EXISTS artists (${columnsDefinition});`;
    await pgClient.query(createTableQuery);
    logger.info(
      `Ensured table 'artists' exists with columns: ${headers.join(", ")}`
    );

    // Prepare the INSERT query.
    const columnsList = headers.map((col) => `"${col}"`).join(", ");
    const placeholders = headers.map((_, i) => `$${i + 1}`).join(", ");
    const insertQuery = `INSERT INTO artists (${columnsList}) VALUES (${placeholders})`;
